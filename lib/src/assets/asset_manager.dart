...
class AssetManager {
  ...
  final _byteCache = <String, Asset<Uint8List>>{};
  final _textCache = <String, Asset<String>>{};
  final _imageCache = <String, Asset<ui.Image>>{};
  final _pendingImages = <String, Future<Asset<ui.Image>>>{};
  ...
  Asset<Uint8List>? bytes(String assetPath, {String? package}) {
    return _byteCache[_resolveKey(assetPath, package: package)];
  }

  Asset<String>? text(String assetPath, {String? package}) {
    return _textCache[_resolveKey(assetPath, package: package)];
  }

  Asset<ui.Image>? image(String assetPath, {String? package}) {
    return _imageCache[_resolveKey(assetPath, package: package)];
  }

  Future<Asset<Uint8List>> loadBytes(
    String assetPath, {
    String? package,
  }) async {
    _ensureNotDisposed();
    ...
    final asset = Asset(data: bytes, path: assetPath, package: package);
    _byteCache[key] = asset;
    return asset;
  }

  Future<Asset<String>> loadText(String assetPath, {String? package}) async {
    _ensureNotDisposed();
    final key = _resolveKey(assetPath, package: package);
    ...
    final asset = Asset(data: value, path: assetPath, package: package);
    _textCache[key] = asset;
    return asset;
  }

  Future<Asset<ui.Image>> loadImage(String assetPath, {String? package}) async {
    _ensureNotDisposed();
    final key = _resolveKey(assetPath, package: package);
    ...
    final future = _decodeImage(
      key,
    ).then((value) => Asset(data: value, path: assetPath, package: package));
